...
@Autonomous
public class FarRedUnderStageDoor extends LinearOpMode {
...
//		Pose2D park = new Pose2D( 44, -12, 0 ); // left
		...
		autoMachine = new StateMachineBuilder()
				...
				.onEnter( () -> robot.followPath( selectedSpike, isRight ? 0 : 270 ) )
				.onExit( () -> {
...
//					robot.intake.setSpinState( Intake.SpinState.SPIKE_SCORE );
				...
				.transition( () -> robot.distanceToTarget() < 0.5 && isRight, AutoStates.RIGHT_INTERMEDIATE_POINT  )
				.transition( () -> robot.distanceToTarget() < 0.5, AutoStates.SCORE_SPIKE  )
				.transitionTimed( 5 )

				.state( AutoStates.DETERMINE_RIGHT )
				.transition( () -> isRight, AutoStates.RIGHT_INTERMEDIATE_POINT  )
				.transition( () -> !isRight, AutoStates.SCORE_SPIKE  )

				.state( AutoStates.RIGHT_INTERMEDIATE_POINT )
