...
public class ColorAndPosDecider {
    public static Pixel decidePixel(ArrayList<Pixel.Color[]> board, int pixelX, int pixelY) {

        Pixel currentPixel = new Pixel(pixelX, pixelY, Pixel.Color.NO_PIXEL);
        ...
        if (board.get(pixelY).length == 6 && pixelY != 0) {
            filledUnderneath = (board.get(pixelY - 1)[pixelX] != Pixel.Color.NO_PIXEL && board.get(pixelY - 1)[pixelX + 1] != Pixel.Color.NO_PIXEL);
        } else if (board.get(pixelY).length == 7 && pixelY != 0) {
            filledUnderneath = (board.get(pixelY - 1)[pixelX] != Pixel.Color.NO_PIXEL && board.get(pixelY - 1)[pixelX - 1] != Pixel.Color.NO_PIXEL); //if you're encountering weird bugs this is where they are
        } else {
            filledUnderneath = true;
        }

        if (currentPixel.color == null && filledUnderneath) {
            if (!canFormMosaic(board, pixelX, pixelY)) {
                return new Pixel(pixelX, pixelY, Pixel.Color.WHITE);
            } else {
                return new Pixel(
                        pixelX,
                        pixelY,
    ...
    private static boolean canFormMosaic(ArrayList<Pixel.Color[]> board, int pixelX, int pixelY) {
        return countNonWhitePixels(board, pixelX, pixelY) == 2;
    }

    private static int countNonWhitePixels(ArrayList<Pixel.Color[]> board, int pixelX, int pixelY) {
        int[][] adjacentCoordinates = generateAdjacentCoordinates(board, pixelX, pixelY);
        int nonWhitePixels = 0;
        for (int i = 0; i < adjacentCoordinates.length; i++) {
            if (board.get(pixelY)[pixelX] == Pixel.Color.WHITE) {
                nonWhitePixels++;
            }
        }
    ...
    private static int[][] generateAdjacentCoordinates(ArrayList<Pixel.Color[]> board, int pixelX, int pixelY) {
    ...
    private static boolean isAdjacent(ArrayList<Pixel.Color[]> board, int pixelX, int pixelY, int pixelX2,
                                      int pixelY2) {
